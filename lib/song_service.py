...
class SongService:
    ...
    def is_cached(self, query: str) -> bool:
        """Return True if this song query is already downloaded in local cache."""
        return self.get_cached(query) is not None

    def get_cached(self, query: str) -> SongDownload | None:
        """Instantly return cached audio if available without disk copying or temp directory allocation."""
        ...
        with self.cache_lock:
            ...
            try:
                ...
                title = cleaned
            return SongDownload(path=audio, title=title, work_dir=None, cache_hit=True)
    ...
    def download(self, query: str) -> SongDownload:
        ...
        # Fast path: instant return for already downloaded songs
