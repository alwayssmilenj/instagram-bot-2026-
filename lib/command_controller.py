...
class CommandController:
    ...
    COMMANDS: dict[str, CommandSchema] = {
        ...
        "remind": CommandSchema(
            name="remind",
            aliases=["remindme", "alarm", "timer", "schedule", "reminders", "snooze"],
            description="Set, list, or snooze scheduled natural language reminders",
            usage=".remindme 10m <text>",
            required_role=UserRole.STANDARD_USER,
            category="utility",
        ...
        "story": CommandSchema(
            name="story",
            aliases=["storygen", "narrative", "storyadventure", "adventure"],
            description="Generate multi-genre short stories or launch interactive choose-your-own-adventure quests",
            usage=".story <genre|prompt> or .story adventure [genre]",
            required_role=UserRole.STANDARD_USER,
            category="interactive",
            handler_name="handle_story",
        ),
        "quotecanvas": CommandSchema(
            name="quotecanvas",
            aliases=["quotecard", "canvasquote", "card"],
            description="Generate aesthetic high-resolution quote cards in 7 distinct visual styles",
            usage=".quotecanvas style:<name> <text>",
            required_role=UserRole.STANDARD_USER,
            category="canvas",
            handler_name="handle_quotecanvas",
        ),
        "vibe": CommandSchema(
            name="vibe",
            aliases=["vibeset", "status", "vibestatus", "vibeboard"],
            description="Broadcast custom live status/mood or view group chat collective vibe board",
            usage=".vibe set <mood> or .vibeboard",
            required_role=UserRole.STANDARD_USER,
            category="interactive",
            handler_name="handle_vibe",
        ),
        "poll": CommandSchema(
            name="poll",
            aliases=["newpoll", "quickpoll", "vote", "pollstatus", "endpoll"],
            description="Interactive group chat polling system with live Unicode percentage bars",
            usage=".poll \"Question\" \"Opt1\" \"Opt2\" or .quickpoll <question>",
            required_role=UserRole.STANDARD_USER,
            category="interactive",
        ...
        "ttt": CommandSchema(
            name="ttt",
            aliases=["tictactoe"],
            description="Play interactive Tic-Tac-Toe against AI or friends with emoji grid",
            usage=".ttt [1-9 | @user | cancel]",
            required_role=UserRole.STANDARD_USER,
            category="games",
            handler_name="handle_ttt",
        ),
        "c4": CommandSchema(
            name="c4",
            aliases=["connect4", "connectfour"],
            description="Play 7x6 Connect Four with gravity drop mechanics",
            usage=".c4 [1-7 | @user | cancel]",
            required_role=UserRole.STANDARD_USER,
            category="games",
            handler_name="handle_c4",
        ),
        "blackjack": CommandSchema(
            name="blackjack",
            aliases=["bj"],
            description="Play Casino Blackjack with Ace flexibility, hit/stand/double",
            usage=".blackjack [bet] | .hit | .stand | .double",
            required_role=UserRole.STANDARD_USER,
            category="games",
            handler_name="handle_blackjack",
        ),
        "tarot": CommandSchema(
            name="tarot",
            aliases=["tarotcard", "tarotreading"],
            description="Draw Major and Minor Arcana cards for mystic guidance",
            usage=".tarot [3 | <question>]",
            required_role=UserRole.STANDARD_USER,
            category="mystic",
            handler_name="handle_tarot",
        ),
        "roastbattle": CommandSchema(
            name="roastbattle",
            aliases=["battle", "roastclash"],
            description="Generate a hilarious multi-round AI roast clash between two users",
            usage=".roastbattle @user1 [@user2]",
            required_role=UserRole.STANDARD_USER,
            category="fun",
            handler_name="handle_roastbattle",
        ),
    }

    INTENT_PATTERNS = [
        (r"\b(?:play\s+tic\s*tac\s*toe|tic\s*tac\s*toe|ttt)\s*(.*)", "ttt"),
        (r"\b(?:play\s+connect\s*4|connect\s*four|c4)\s*(.*)", "c4"),
        (r"\b(?:play\s+blackjack|blackjack|deal\s+cards|play\s+bj)\s*(.*)", "blackjack"),
        (r"\b(?:tarot\s+card|tarot\s+reading|draw\s+a\s+tarot\s+card|tarot)\s*(.*)", "tarot"),
        (r"\b(?:roast\s+battle|roast\s+clash|battle\s+between)\s+(.+)", "roastbattle"),
